...
| `VCKNOTS_AUTHZ_HTTP_ALLOWED` | Issuer & Authz | When set to `true`, an HTTP endpoint is allowed for the Authorization Server Issuer (for local development/testing) |
...
Authorization Server Issuer identifiers require HTTPS by default and cannot contain query or fragment components. The local `.env.example` sets `VCKNOTS_AUTHZ_HTTP_ALLOWED=true` because the local Issuer and Authz URLs default to HTTP. Do not enable `VCKNOTS_AUTHZ_HTTP_ALLOWED` in production.
